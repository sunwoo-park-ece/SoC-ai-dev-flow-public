...
## P09B slot-3 contract — paired publication update

> **Current Public integration:** This implementation is current with the paired P09B source/documentation commits. Earlier candidate wording in this section is pre-publication provenance only.

> **Publication synchronization:** Apply this implemented slot-3 contract only with the matching P09B source commit; it does not assert a final Public commit SHA before read-back.

The isolated P09B candidate expands only canonical slot 3 to aligned word offsets `+0x00/+0x04/+0x08/+0x0C/+0x10`. The bridge validates the full address, size, alignment and listed offset **before** selecting the wrapper; mirrors and every other offset remain ERROR with no wrapper side effect. The wrapper checks read/write direction and exact SNAP_CTRL command encoding, raises PSLVERR only on completing ACCESS for invalid requests, and otherwise remains zero-wait. Top-level PSLVERR reaches the existing bridge two-cycle AHB ERROR path. An eligible CAPTURE is one command per completed ACCESS; an ineligible but well-encoded CAPTURE is OKAY/no-op. STATUS is read-only and side-effect-free, with pre-edge registered readback on a same-edge sample publication. The exact ABI and priorities are in [12_gsensor.md](12_gsensor.md). No new bus slot, PLIC route or generic SPI aperture is allocated. These are isolated-candidate facts, not current Public `main` claims.

The isolated candidate uses one common reset: G-sensor controller `iRSTN`, snapshot banks and scheduler use bridge-provided `PRESETn=HRESETn` directly, without the extra local delay. Normal ACCESS/pre-edge read rules apply only while reset is deasserted. Asynchronous assertion can overlap SETUP or ACCESS and abort an unfinished transfer; it must not create an accepted CAPTURE or a guaranteed read response. A read completed before assertion remains a historical completion. This does not alter current Public `main` or promise a particular response code for reset-aborted traffic.
