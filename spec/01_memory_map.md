...
## P09B G-sensor allocation — paired publication update
...
> **Publication synchronization:** This contract becomes the Public contract only when the matching P09B source and documentation commits are both published. Until then the pinned Public main remains historical.

The canonical G-sensor base remains `0x4003_0000` (APB slot 3). The isolated P09B candidate implements exactly five naturally aligned 32-bit register offsets: `+0x00` HOLD_XY (read), `+0x04` HOLD_Z (read), `+0x08` STATUS (read), `+0x0C` HOLD_SEQ (read), and `+0x10` SNAP_CTRL (write). Their direction, validity, zero-gated HOLD readback and exact commands are specified in [12_gsensor.md](12_gsensor.md). All other offsets, aliases, unsupported sizes and wrong-direction accesses fault through the existing two-cycle AHB ERROR path before wrapper side effect. This is not a claim about current Public `main`, which remains at the pinned base until integration is approved. No APB aperture or neighboring slot changes.
