...
| `CDC-001` | High | BC | VGA `front_buffer_idx` crosses HCLK→pclk25 directly. | Frame-safe CDC/ownership handshake with a defined commit boundary. | `06_reset_clock`, `08_vga` | asynchronous-clock swap tests | OPEN |
...
| ID | Sev | Gate | Current issue | Required outcome | Dependent specs | Verification | Status |
|---|---|---|---|---|---|---|---|
| `VGA-001` | High | BC | Full `0x2xxx_xxxx` selection / local aliases. | Restrict decode to canonical framebuffer/status/control apertures. | `01_memory_map`, `04_ahb_fabric`, `08_vga` | boundary/alias-negative tests | OPEN |
| `VGA-002` | Medium | BC | No framebuffer read path; subword helper exists without byte-enable support. | Implement frozen A3: write-only aligned 32-bit framebuffer; unsupported reads/subword accesses ERROR; remove/deprecate byte-write helper. | `08_vga`, `19_firmware_contract` | read/size negative tests + firmware search | OPEN |
| `VGA-003` | High | BC | CPU writes during `CLEAR_BUSY` are acknowledged but discarded. | Backpressure/reject/error/ownership mechanism that removes false success. | `08_vga`, `04_ahb_fabric` | clear/write contention | OPEN |
| `VGA-004` | High | BC | Cleaner target follows live front-buffer selection. | Latch clear target at start; define repeated-command behavior. | `08_vga` | SWAP during clear | OPEN |
| `VGA-005` | Medium | BC | `CLEAR_DONE` is high in IDLE/DONE, not a clean event. | Use BUSY semantics or define a sticky completion event. | `08_vga`, `19_firmware_contract` | reset/start/done tests | OPEN |
| `VGA-006` | High | BC | HW-clear board acceptance is missing from cleanup signoff. | Add RTL + physical HW-clear acceptance after arbitration/CDC fixes. | `08_vga` | 9600-word clear + board test | OPEN |
...
|---|---|---|---|---|---|---|
| `FW-001` | High | BC | Deprecate/remove unsupported peripheral/VRAM byte-write usage under frozen A3 word-only framebuffer contract. | `APB-006`, `VGA-002` | code search + framebuffer test | OPEN |
...
| `FW-004` | High | BC | Timer driver and active applications use the frozen A5 fresh one-shot sequence. | `TIMER-001..003` | host MMIO ordering + public `final_main` RV32I build PASS; approved historical local `benchmark_main` build retained privately because its source is excluded from publication | VERIFIED |
...
| A3 VGA | Framebuffer write-only, aligned 32-bit; read/subword/gap/alias ERROR. | Phase 4A-3A bus access policy enforced; `VGA-002` and `FW-001` full cleanup remain OPEN. |
