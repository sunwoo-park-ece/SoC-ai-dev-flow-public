...
// S1: CTRL[31:2] is RAZ/WI.  Expected values are derived only from the
// published register/segment contract, not from DUT state.
module tb_hex_s1_ctrl_razwi;
    ...
    task automatic fail(input string message);
        $fatal(1, "S1_CHECK_FAIL: %s", message);
    endtask
    ...
    task automatic expect_read(input logic [31:0] address, input logic [31:0] expected,
        ...
        begin
            oracle = expected;
            if (inject_bad_expectation && check_id == "S1_MUTATION_TARGET") oracle = oracle ^ 32'h1;
            @(negedge clk);
    ...
    task automatic expect_blank(input string check_id);
        begin
            ...
            if ({hex5, hex4, hex3, hex2, hex1, hex0} !== {6{BLANK}})
                fail({check_id, " expected blank HEX0..HEX5"});
        end
    endtask
    ...
    task automatic write_ctrl_reserved(input logic [1:0] functional_bits, input string check_id);
        begin
            apb_write(CTRL, 32'hffff_fffc | functional_bits, check_id);
            expect_read(CTRL, {30'b0, functional_bits}, check_id);
        end
    endtask
    ...
    initial begin
        ...
        resetn = 1'b1;
        repeat (2) @(posedge clk);
        expect_read(CTRL, 32'h0000_0001, "S1_RESET_CTRL");
        expect_decoded(24'h000000, "S1_RESET_DECODED");

        // Establish distinct decoder/raw side effects before enumerating CTRL states.
        apb_write(VALUE, 32'h00ab_cdef, "S1_WRITE_VALUE");
        apb_write(RAW_LOW, 32'h0015_5555, "S1_WRITE_RAW_LOW");
        apb_write(RAW_HIGH, 32'h000a_aaaa, "S1_WRITE_RAW_HIGH");

        // Every write has reserved bits set; each [1:0] combination must update.
        write_ctrl_reserved(2'b00, "S1_CTRL_00_RAZWI");
        expect_blank("S1_CTRL_00_DISABLE");
        write_ctrl_reserved(2'b01, "S1_CTRL_01_RAZWI");
        expect_decoded(24'hab_cdef, "S1_CTRL_01_DECODE");
        write_ctrl_reserved(2'b10, "S1_CTRL_10_RAZWI");
        expect_blank("S1_CTRL_10_DISABLE_RAW");
        write_ctrl_reserved(2'b11, "S1_CTRL_11_RAZWI");
        expect_raw(21'h155555, 21'h0aaaaa, "S1_CTRL_11_RAW");

        // Isolated checker mutation: a correct CTRL read must reject a false oracle.
        expect_read(CTRL, 32'h0000_0003, "S1_MUTATION_TARGET");

        // Reset while raw mode is active restores enabled decoder mode and reset data.
        @(negedge clk); resetn = 1'b0;
        @(posedge clk); #1;
        resetn = 1'b1;
        repeat (2) @(posedge clk);
        expect_read(CTRL, 32'h0000_0001, "S1_RESET_AFTER_RAW_CTRL");
        expect_read(RAW_LOW, 32'h001f_ffff, "S1_RESET_AFTER_RAW_LOW");
        expect_read(RAW_HIGH, 32'h001f_ffff, "S1_RESET_AFTER_RAW_HIGH");
        expect_decoded(24'h000000, "S1_RESET_AFTER_RAW_OUTPUT");

        if (inject_bad_expectation) fail("INJECT_BAD_EXPECTATION did not reach S1 checker");
        $display("SUMMARY: PASS HEX S1 CTRL RAZ/WI checks=%0d", checks);
        $finish;
    end
endmodule
