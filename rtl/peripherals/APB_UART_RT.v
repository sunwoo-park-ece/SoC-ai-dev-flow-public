// APB UART slave with 8N1 TX/RX and a 16-byte RX FIFO.
module APB_UART (
    input  wire        PCLK,
    input  wire        PRESETn,
    ...
    // A DATA write remains in APB ACCESS while the transmitter is occupied.
    // The write is accepted once, on the cycle where PREADY is observed high.
    ...
    always @(*) begin
        ...
        if (apb_access && !PWRITE) begin
            case (PADDR[15:0])
                ...
                UART_STATUS:
                    PRDATA = {29'h0, rx_error, rx_ready, !tx_busy};
                UART_CONTROL:
                    PRDATA = 32'h0; // Reserved: read-as-zero/write-ignored.
                UART_BAUD:
    ...
    // TX_ACTIVE captures the programmed divisor with the accepted DATA write.
    // The ready indication rises on the edge completing the stop-bit interval.
    always @(posedge PCLK or negedge PRESETn) begin
        if (!PRESETn) begin
    ...
    // RX_ACTIVE captures the programmed divisor at start-edge detection. A
    // simultaneous BAUD write therefore affects only the following frame.
    always @(posedge PCLK or negedge PRESETn) begin
        if (!PRESETn) begin
            ...
            // A newly detected error wins over a same-cycle W1C request.
