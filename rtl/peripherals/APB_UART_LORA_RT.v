// APB LoRa UART 슬레이브 모듈 (Integrated RX/TX + LoRa AUX status).
module APB_UART_LORA (
    ...
    input  wire        lora_aux, // LoRa AUX 입력핀
    ...
    localparam STAT_AUX_HIGH = 3; // 1 = LoRa AUX High (module ready)
    ...
    // LoRa 수신은 RF 모듈이 여러 바이트를 burst로 밀어낼 수 있으므로 FIFO가 필요합니다.
    ...
    // LoRa AUX input synchronizer
    reg aux_sync_1, aux_sync_2;
    ...
    // LoRa AUX Synchronizer 2-ff 동기화기
    always @(posedge PCLK or negedge PRESETn) begin
        if (!PRESETn) begin
            aux_sync_1 <= 1'b0;
            aux_sync_2 <= 1'b0;
    ...
    always @(*) begin								
			if (PSEL && PENABLE && !PWRITE) begin
                case (PADDR[3:2])
                    ...
                    UART_STATUS:  PRDATA = {28'h0, eff_aux, rx_error, rx_ready, !tx_busy}; 
    ...
    // LoRa AUX는 UART_STATUS[3]으로 노출하며, 전송 전 AUX high 폴링은 펌웨어에서 수행합니다.
    // -------------------------------------------------------------------------
    always @(posedge PCLK or negedge PRESETn) begin
        if (!PRESETn) begin
            tx_busy      <= 1'b0;
